...
interface MessageQueue : Library {
    ...
    fun mq_receive(__mqdes: Int, __msg_ptr: Pointer, __msg_len: SIZE_T, __msg_prio: Uint32_t?): SSIZE_T
...
class MQ(private val name: String, private val mode: Mode, private val isManaging: Boolean = false) {
    ...
    fun receive(): String? {
        ...
        val buffer = Memory(MESSAGE_BUFFER_SIZE)
        val msgSize = mq.mq_receive(mqDescriptor, buffer, SIZE_T(MESSAGE_BUFFER_SIZE), null)
        try {
            if (msgSize.toInt() > 0) {
                return buffer.getByteArray(0, msgSize.toInt()).decodeToString()
            }
        } catch (_: Exception) {
        }
        return null
