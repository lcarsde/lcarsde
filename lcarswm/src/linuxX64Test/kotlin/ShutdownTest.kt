...
import de.atennert.mock.drawing.MockDrawFactory
...
class ShutdownTest : SystemCallMocker() {
    ...
    override fun setup() {
        ...
        ServiceLocator.provide<Logger> {
            LoggerMock()
                .closeWith(LoggerMock::close)
        }
        ServiceLocator.provide<DrawFactory> { MockDrawFactory }
    }
        ...
        val testFacade = object : SystemFacadeMock() {
            ...
            override fun getSelectionOwner(atom: Atom): Window {
                return if (atom == atomMap["WM_S${defaultScreenNumber()}"]) {
                    super.functionCalls.add(
                        FunctionCall(
                            "getSelectionOwner",
                            atom,
                            selectionOwnerCounter++
                        )
                    )
                    None.convert()
                } else {
    ...
    private fun checkCleanupOfSignals(
        functionCalls: MutableList<FunctionCall>,
        registeredSignals: Set<Signal>
    ) {
