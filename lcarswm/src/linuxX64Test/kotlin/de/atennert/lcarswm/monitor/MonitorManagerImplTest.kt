...
class MonitorManagerImplTest {
    ...
    fun `update monitor list`() {
        ...
        assertEquals(
            systemApi.outputs.toList(),
            monitors.last().map { it.id },
            "The monitor IDs should match"
        )
        assertEquals(
            systemApi.outputNames.toList(),
            monitors.last().map { it.name },
            "The monitor names should match"
        )
        assertEquals(
            systemApi.primaryOutput,
            primaries.last()?.id,
            "The ID of the primary monitor should match"
        )
    ...
    fun `check the first monitor becomes primary if there's no primary`() {
        ...
        assertEquals(
            monitors.last()[0].id,
            primaries.last()?.id,
            "The first monitor shall become the primary, if there's no primary"
        )
    ...
    fun `don't add monitors without crtc`() {
        ...
        assertContentEquals(
            listOf(systemApi.outputs[0]),
            monitors.last().map { it.id },
            "The monitor with crtc should be in the monitor list ${monitors.last()}"
        )
