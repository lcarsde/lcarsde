...
class SignalHandlerTest {
    ...
    fun `handle core signal`() {
        val system = _root_ide_package_.de.atennert.lcarswm.system.SystemFacadeMock()
        ...
        system.signalActions
            ...
            .forEach { (signal, _) ->
                ...
                _root_ide_package_.kotlin.test.assertEquals("abort", abortCall.name, "Call abort for signal $signal")
            }
    }
    ...
    fun `add signal callback`() {
        ...
        signalHandler.addSignalCallback(
            Signal.INT,
            staticCFunction<Int, Unit> { receivedSignal = it })
        ...
        _root_ide_package_.kotlin.test.assertEquals(
            ...
            receivedSignal,
            "We should get the signal, that we registered for"
        )
    }
}
