...
class KeyReleaseHandlerTest : SystemCallMocker() {
    ...
    fun `kill active window if the WM protocols result is != 0 on F4`() {
        ...
        val focusHandler = WindowFocusHandler(
            windowList,
            AppMenuMessageHandler(systemApi, AtomLibrary(systemApi), windowList)
        )
        ...
        assertEquals(
            "killClient",
            killClientCall.name,
            "The window should be killed if we can't get its protocols"
        )
        assertEquals(
            windowId,
            killClientCall.parameters[0],
            "The active window needs to be killed"
        )
    }
    ...
    fun `send delete window event on F4 when client supports it`() {
        ...
        assertEquals(
            "sendEvent",
            xClientCall.name,
            "The WM should send a delete window request"
        )
        assertEquals(
            windowId,
            xClientCall.parameters[0],
            "The delete request should be for the right window"
        )
        assertFalse(xClientCall.parameters[1] as Boolean, "The event should not propagate")
        assertEquals(
            0.toLong(),
            xClientCall.parameters[2],
            "The mask should not contain anything"
        )
        ...
        assertEquals(
            atomLibrary[Atoms.WM_PROTOCOLS],
            eventData.xclient.message_type,
            "The message type should be WM_PROTOCOLS"
        )
        assertEquals(32, eventData.xclient.format, "The message format should be 32 bit")
        assertEquals(
            atomLibrary[Atoms.WM_DELETE_WINDOW],
            eventData.xclient.data.l[0].convert(),
            "The delete WM window atom needs to match"
        )
    }
